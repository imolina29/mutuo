...
export async function DELETE(req: NextRequest) {
  ...
  // --- Step 3: Nullify user references in declarations (preserve legal record) ---
  // For declarations where user is the invited party, remove the reference
  await db.declaration.updateMany({
    where: { invitedId: user.id },
    data: { invitedId: null },
  });

  // --- Step 4: Log deletion BEFORE deleting user (audit trail for compliance) ---
  ...
  await logAudit({
    ...
    details: {
      ...
      emailHash: Buffer.from(email).toString("base64"), // pseudonymized reference
    },
    ...meta,
  });

  // --- Step 5: Delete user (cascades to identity, notifications, blocks, reports) ---
  ...
  try {
      ...
      html: `
        <div style="font-family: Inter, sans-serif; max-width: 600px; margin: 0 auto;">
          ...
          <ul>
            ...
            <li>Declaraciones selladas donde participaste como creador (requerimiento legal Ley 527/1999)</li>
          </ul>
    ...
    // Non-blocking — the deletion already happened
  }
